...
**Correcting and withdrawing notes** — both kinds of note can be fixed or taken back from MCP as well as from the API, so an agent that logged something wrong can clean up after itself without a human opening the browser. `update_note`/`delete_note` take `id` (the application) and `note_id`; `update_contact_note`/`delete_contact_note` take `contact_id` and `note_id`. The edits are patches — an omitted field stands. Every delete here is a hide and has no undo, so prefer the update tool when a note is wrong rather than unwanted. A note that is already hidden, or belongs to another user, is a 404 either way: the error never confirms that the note exists.
