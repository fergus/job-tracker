...
- `contact_notes` — interaction log per contact: `content` plus `occurred_at` (when it happened, not when it was typed), `updated_at` (null until the note is corrected) and `hidden_at`; cascade-deletes with the contact
...
- `stage_notes` — per-application notes with stage and markdown content, plus `updated_at` and `hidden_at`
- **Deleting a note of either kind is a hide, not a delete.** Both note tables carry a nullable `hidden_at`, and a delete stamps it rather than removing the row. A hidden note is absent from every read — the API, MCP, the context assembled for document generation, and the prose carried across when an application is converted into a contact — and no tool, endpoint or flag brings it back. The row surviving *is* the recovery story; getting one back means going into the database by hand. Every stage-note read goes through `visibleStageNotes` / `visibleStageNote` in `server/services/stage-notes.js`, so add new readers there rather than writing a fresh `SELECT`. The one deliberate exception is the conversion backup, which captures every row including hidden ones, because a backup that omits them would not make the conversion reversible.
...
- **One follow-up model, shared.** `server/lib/followup.js` owns the instance timezone (`INSTANCE_TIMEZONE`, falling back to `TZ` then UTC) and the `overdue`/`due`/`upcoming` derivation. Contacts and applications both consume it; neither adds a second. Never recompute the classification in a caller — the API returns `follow_up_state` and `follow_up_days`. Applications carry `next_action_at` (a calendar date, set or cleared with `null` on create/update) and filter by state via `follow_up_state` on `GET /api/applications` (comma-separated) and `list_applications` (array) — use `overdue,due` for what needs chasing. A write whose only field is `next_action_at` does not bump `updated_at`, so `updated_since` polling will not see a date-only change; query by `follow_up_state` instead.
- **`last_contacted_at` is derived from the interaction log, never typed.** `add_contact_note` advances it, and only forwards, so writing up an old call cannot make a relationship look staler than it is. Editing and deleting an interaction obey the same rule: re-dating one forwards advances the date, but re-dating it backwards leaves it alone, and withdrawing one never moves it. A contact can therefore show a last-contacted date that no visible interaction accounts for. That is deliberate, not drift — the field is maintained by comparison, and nothing recomputes it from `MAX(occurred_at)` over the surviving log. The alternative was considered and rejected: no write should be able to make a relationship look staler than it already looked.
...
- The status split backfill runs on startup in report mode and writes nothing. Set `SCHEMA_BACKFILL=apply` to apply it once. See `docs/plans/completed/2026-08-12-002-refactor-pipeline-schema-split-plan.md`.
...
**Correcting and withdrawing notes** — both kinds of note can be fixed or taken back from MCP as well as from the API, so an agent that logged something wrong can clean up after itself without a human opening the browser. `update_note`/`delete_note` take `id` (the application) and `note_id`; `update_contact_note`/`delete_contact_note` take `contact_id` and `note_id`. The two edits do not have the same shape, and the difference will bite an agent that assumes they do: `update_contact_note` is a true patch, where an omitted field stands and at least one of `content`/`occurred_at` must be given, while `update_note` **replaces** the note's content and requires it on every call — `stage` is its only optional field, so re-filing a note under a different stage still means resending its text unchanged. Every delete here is a hide and has no undo, so prefer the update tool when a note is wrong rather than unwanted. A note that is already hidden, or belongs to another user, is a 404 either way: the error never confirms that the note exists.
