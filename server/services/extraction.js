...
const { MIME_MAP } = require("../lib/mime");
...
async function extractText(filePath, mimeType, meta = {}) {
    ...
    const effectiveMime = mimeType || MIME_MAP[ext] || null;
    const logPrefix =
        meta.attachmentId !== undefined
            ? `[extraction][id=${meta.attachmentId}]`
            : `[extraction]`;

    console.log(
        `${logPrefix} start path="${filePath}" ext="${ext}" mime="${effectiveMime || "unknown"}"`,
    );
    ...
    try {
        if (effectiveMime === "application/pdf" && pdfParse) {
            ...
            if (text) {
                console.log(
                    `${logPrefix} success format=pdf length=${text.length}`,
                );
            } else {
                console.log(
                    `${logPrefix} success format=pdf length=0 (empty document)`,
                );
            }
            return text;
        }
        ...
        if (
            ...
            if (text) {
                console.log(
                    `${logPrefix} success format=docx length=${text.length}`,
                );
            } else {
                console.log(
                    `${logPrefix} success format=docx length=0 (empty document)`,
                );
            }
            return text;
        }
        ...
        if (ext === ".doc" && mammoth) {
            ...
            if (text) {
                console.log(
                    `${logPrefix} success format=doc length=${text.length}`,
                );
            } else {
                console.log(
                    `${logPrefix} success format=doc length=0 (empty document)`,
                );
            }
            return text;
        }
        ...
        if (
            ...
            if (text) {
                console.log(
                    `${logPrefix} success format=text length=${text.length}`,
                );
            } else {
                console.log(
                    `${logPrefix} success format=text length=0 (empty file)`,
                );
            }
            return text;
        }
        ...
        console.log(
            `${logPrefix} skipped reason="unsupported type" ext="${ext}" mime="${effectiveMime || "unknown"}"`,
        );
        return null;
    } catch (err) {
...
async function extractStructuredJD(text) {
    ...
    if (!client) {
        console.error(
            '[jd-extraction] failed reason="OPENAI_API_KEY not configured"',
        );
        return null;
    }
    ...
    console.log(
        `[jd-extraction] start model="${model}" text_length=${text.length}`,
    );
    ...
    try {
        ...
        if (!raw) {
            console.error(
                '[jd-extraction] failed reason="empty response from LLM"',
            );
            return null;
        }
        ...
        const result = {
            required_skills: Array.isArray(parsed.required_skills)
                ? parsed.required_skills.filter(Boolean)
                : [],
            responsibilities: Array.isArray(parsed.responsibilities)
                ? parsed.responsibilities.filter(Boolean)
                : [],
            experience_requirements: Array.isArray(
                parsed.experience_requirements,
            )
                ? parsed.experience_requirements.filter(Boolean)
                : [],
            salary_signals:
                parsed.salary_signals &&
                typeof parsed.salary_signals === "object"
                    ? {
                          min:
                              typeof parsed.salary_signals.min === "number"
                                  ? parsed.salary_signals.min
                                  : null,
                          max:
                              typeof parsed.salary_signals.max === "number"
                                  ? parsed.salary_signals.max
                                  : null,
                          currency:
                              typeof parsed.salary_signals.currency === "string"
                                  ? parsed.salary_signals.currency
                                  : null,
                          period:
                              typeof parsed.salary_signals.period === "string"
                                  ? parsed.salary_signals.period
                                  : null,
                      }
            ...
            employment_type:
                typeof parsed.employment_type === "string"
                    ...
                    : null,
            seniority_level:
                typeof parsed.seniority_level === "string"
                    ? parsed.seniority_level
                    : null,
        };

        console.log(
            `[jd-extraction] success skills=${result.required_skills.length} responsibilities=${result.responsibilities.length}`,
        );
        return result;
    } catch (err) {
