...
const db = require("../db");
const { apiKeySecret } = require("../lib/apiKeySecret");
...
function cachedUpsertUser(email) {
    ...
    try {
        db.prepare(
            "INSERT OR IGNORE INTO user_profiles (user_email) VALUES (?)",
        ).run(email);
    } catch (err) {
        console.error(
            "[auth] failed to auto-create profile for %s:",
            email,
            err.message,
        );
    }
...
function authMiddleware(req, res, next) {
    ...
    if (process.env.NODE_ENV === "production") {
        ...
        if (!req.headers["x-forwarded-user"] || !email) {
            console.warn(
                "[auth] rejected OAuth request: missing forwarded headers from %s",
                req.ip,
            );
