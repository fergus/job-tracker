...
function requireOAuth(req, res, next) {
  if (req.authMethod !== 'oauth') {
    return res.status(403).json({ error: 'Profile management requires browser authentication' });
  }
  next();
}
...
router.get('/me/profile', (req, res) => {
  const profile = db.prepare('SELECT * FROM user_profiles WHERE user_email = ?').get(req.userEmail);
  if (!profile) {
...
router.put('/me/profile', requireOAuth, (req, res) => {
  ...
  for (const field of ALLOWED_FIELDS) {
    if (req.body[field] !== undefined) {
      if ((field === 'linkedin_url' || field === 'portfolio_url') && !isValidUrl(req.body[field])) {
  ...
  db.prepare(`UPDATE user_profiles SET ${updates.join(', ')} WHERE user_email = ?`).run(...values);
...
router.get('/users/:email/profile', (req, res) => {
  ...
  const profile = db.prepare('SELECT * FROM user_profiles WHERE user_email = ?').get(email);
  if (!profile) {
    return res.status(404).json({ error: 'Profile not found' });
  }
